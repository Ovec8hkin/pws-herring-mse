...
fun_write_dat <- function(dat.files, catches, obs, params, ss, year){
  ...
  if(-9 %in% obs$seine.age.comp){ss$seac <- 0}
  if(-9 %in% obs$spawn.age.comp){ss$spac <- 0}
  ...
  PWS.ASA.dat[[3]]  <- 10                                                         # Number of age classes (constant)
  PWS.ASA.dat[[4]]  <- rbind(PWS.ASA.dat[[4]],  obs$waa)                          # Weight-at-age (assumed constant 03/2022) 
  PWS.ASA.dat[[5]]  <- rbind(PWS.ASA.dat[[5]],  obs$fec)                          # Fecundity (no data post 1997) 
  ...
  PWS.ASA.dat[[15]] <- 26                                                         # First year of ADFG hydroacoustic survey (constant)
  PWS.ASA.dat[[16]] <- rbind(PWS.ASA.dat[[16]], obs$adfg.hydro)                   # Observed ADFG hydroacoustic survey biomass
  PWS.ASA.dat[[17]] <- 14                                                         # FIrst year of PWSSC hydroacoustic survey (constant)
  ...
  age.comp.samp.sizes.txt[[1]] <- rbind(age.comp.samp.sizes.txt[[1]], ss$seac)
  age.comp.samp.sizes.txt[[2]] <- rbind(age.comp.samp.sizes.txt[[2]], ss$spac)
